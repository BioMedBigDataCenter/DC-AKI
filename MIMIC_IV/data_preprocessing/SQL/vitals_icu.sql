...
drop table if exists vital_all_icu_0721;
create table vital_all_icu_0721 as (
...
-- 	, max(case when VitalID = 8 then valuenum ELSE NULL END) AS glucose_max
	...
	FROM  (
	  select smp.subject_id, smp.hadm_id, smp.stay_id, ce.charttime
	  , case
		...
		when ce.itemid in (223761,678) and ce.valuenum > 70 and ce.valuenum < 120  then 6 -- TempF, converted to degC in valuenum call
		...
		when ce.itemid in (807,811,1529,3745,3744,225664,220621,226537) and ce.valuenum > 0 then 8 -- Glucose
		  ...
		  -- convert F to C
	  , case when ce.itemid in (223761,678) then (ce.valuenum-32)/1.8 else ce.valuenum end as valuenum
		
	  from sample_inclusion_0721 smp
	  left join chartevents ce
		  on smp.stay_id = ce.stay_id
		  and (ce.charttime between smp.icu_intime and smp.icu_outtime)
		  and ce.itemid in
		  (
		  -- HEART RATE
		  211, --"Heart Rate"
